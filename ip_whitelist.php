<?php
/**
 * Plugin Management: PI Whitelist
 * Date: 2025/11/21
 * File name: ip_whitelist.php
 *
 */
...
function ipWhitelistActions($obj_db, $array_data) {
    global $php_self;    
...
if (isset($_POST['itemID']) AND !empty($_POST['itemID']) AND isset($_POST['itemAction'])) {
    ...
    foreach ($_POST['itemID'] as $itemID) {
        ...
        if (!$sql_op->delete('ip_whitelist', "id='$itemID'")) {
            $error_num++;
        }
    }

    if ($error_num == 0) {
        utility::jsAlert(__('All selected IP addresses successfully removed from whitelist.'));
...
if (isset($_POST['saveData']) AND $can_write) {
    ...
    $new_notes = trim($dbs->escape_string($_POST['notes']));

    if (empty($new_notes)) {
        utility::jsAlert(__('Notes cannot be empty!'));
        exit();
    }

    $data['notes'] = $new_notes;
    
    $update = $sql_op->update('ip_whitelist', $data, 'id=\''.$updateRecordID.'\'');

    if ($update) {
        utility::jsToastr(__('IP Whitelist'), __('Data Successfully Updated'), 'success');
        echo '<script type="text/javascript">parent.jQuery(\'#mainContent\').simbioAJAX(\''.$clean_self_url.'\');</script>';
    } else { 
        utility::jsToastr(__('IP Whitelist'), __('Data FAILED to Updated. Please Contact System Administrator')."\nDEBUG : ".$sql_op->error, 'error'); 
        echo '<script type="text/javascript">parent.jQuery(\'#mainContent\').simbioAJAX(\''.$clean_self_url.'\');</script>';
    }
    exit();
}
...
<div class="menuBoxInner masterFileIcon">
    <div class="per_title">
        <h2><?php echo __('IP Whitelist'); ?></h2>
    </div>
    <div class="infoBox">
        <?= __('This page lists IP addresses permanently allowed to bypass temporary blocking checks. Deleting an entry will subject the IP to security checks again.') ?>
    </div>
...
if (isset($_GET['action']) AND $_GET['action'] == 'detail') {
    ...
    $rec_q = $dbs->query("SELECT * FROM ip_whitelist WHERE id='$itemID'");
    ...
    $form->addAnything(__('Whitelisted Date'), $rec_d['created_at']);
    $form->addTextField('text', 'notes', __('Notes').'*', $rec_d['notes']??'', 'class="form-control col-12"', __('Additional notes for whitelisting this IP.'));
    if ($form->edit_mode) {      
    echo '<div class="infoBox">'.__('You are going to edit data').' : <b>'.$rec_d['ip_address'].'</b></div>';
    }    
    echo $form->printOut();
} else {
    $table_spec = 'ip_whitelist AS t';
    $datagrid = new simbio_datagrid();
    $datagrid->setSQLColumn('t.id',
        't.ip_address',
        't.notes AS \''.__('Notes').'\'',
        't.created_at AS \''.__('Whitelisted Date').'\'');
    $datagrid->setSQLorder('t.created_at DESC');
    if (isset($_GET['keywords']) AND $_GET['keywords']) {
        $keywords = utility::filterData('keywords', 'get', true, true, true);
        $datagrid->setSQLCriteria("t.ip_address LIKE '%$keywords%' OR t.notes LIKE '%$keywords%'");
    }
    $datagrid->modifyColumnContent(1, 'callback{ipWhitelistActions}');
